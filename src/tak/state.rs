...
impl State {
    ...
    pub fn to_tps(&self) -> String {
        ...
        for y in (0..board_size).rev() {
            ...
            for x in 0..board_size {
                if self.board[x][y].is_empty() {
                    ...
                    for piece in self.board[x][y].iter() {
                        match piece.get_color() {
                            Color::White => tps.push('1'),
                            Color::Black => tps.push('2'),
                        }

                        match piece {
                            &Piece::StandingStone(_) => tps.push('S'),
                            &Piece::Capstone(_) => tps.push('C'),
                            _ => (),
                        }
                    }
        ...
        tps.push(' ');
        tps.push_str(&(self.ply_count / 2 + 1).to_string());
        tps.push_str("\"]");
